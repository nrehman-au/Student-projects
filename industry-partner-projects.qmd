...
**Methods and code.** Start with [multinomial logistic regression](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html) to rank the known candidate configurations using vibration and operational features. If raw spectra or time series are available, compare it with [MiniRocket](https://arxiv.org/abs/2012.08791), for which [open-source code](https://github.com/angus924/minirocket) is available. As an optional physics-informed extension, use shaft speed and component geometry to construct order-domain or characteristic-frequency features, following the general approach demonstrated by [Shen et al](https://doi.org/10.1016/j.engappai.2021.104295).

**Baselines and evaluation.** Compare three inputs: operational metadata only, vibration features only, and their combination. Split the data by turbine. Evaluate whether the correct configuration is ranked first or appears among the highest-ranked candidates, and use a [reliability diagram](https://scikit-learn.org/stable/modules/calibration.html) to check whether the reported confidence agrees with actual accuracy.
