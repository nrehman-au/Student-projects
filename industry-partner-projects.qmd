...
**Method and code.** Extract spectral features from the vibration measurements and combine them with available operational data. Train a simple probabilistic model, such as [multinomial logistic regression](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html), to assign a probability to each candidate drivetrain configuration. If raw spectra or time series are available, compare it with [MiniRocket](https://arxiv.org/abs/2012.08791), for which [open-source code](https://github.com/angus924/minirocket) is available. As a physics-informed extension, include available bearing or drivetrain kinematic information in the model and test whether it improves the configuration ranking, particularly for components with limited labelled data.

**Evaluation**: Compare models using operational data only, vibration features only, and both together, and evaluate them on turbines not used for training. Assess whether the correct configuration is ranked first or among the leading candidates and whether the reported confidence is reliable.
