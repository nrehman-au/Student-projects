...
**Project goal.** A practically useful outcome may be narrowing the set of plausible configurations and providing calibrated confidence estimates, rather than always identifying a single configuration with certainty. From an industrial perspective, reducing uncertainty can already provide significant value, even if perfect identification is not possible. The study should also identify which signal characteristics carry useful configuration information.
...
**Practical notes.** This project is feasible only if enough turbines have reliable configuration labels for training and validation. Configuration may also correlate with site, turbine age or rating, so these variables must be examined before attributing performance to vibration signatures. A physics-informed transfer study from well-labelled components to complex components with limited labels is a valuable MSc extension, but it depends on having both known kinematic parameters and a credible target-component test set.
